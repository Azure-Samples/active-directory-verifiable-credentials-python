...
if ( $ClientCertificate ) {
    ...
    if ( $False -eq $isMacLinux ) {
        ...
        $certData = Get-Content $certLocation | Out-String
...
$dictionary = @{ "azTenantId" = $tenantId; "azClientId" = $clientAadApplication.AppId; "azClientSecret" = $client_secret; 
    "azCertificateName" = $certSubject; "azCertificateLocation" = $certLocation.Replace("\", "\\"); "azCertificatePrivateKeyLocation" = $certPrivateKey.Replace("\", "\\")
};
